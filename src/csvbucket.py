...
def function(filename):
	...
	df['unixstarttime']=pd.to_datetime(df.startDateTime, format='%Y-%m-%dT%H:%M:%S').apply(getvalue)
	df['unixstoptime']=pd.to_datetime(df.stopDateTime, format='%Y-%m-%dT%H:%M:%S').apply(getvalue)
